...
export default function App() {
  ...
  useEffect(() => {
    ...
    socketRef.current.on('profiles_update', (profiles) => {
      ...
      if (activeMatch) {
        ...
        if (activeMatch.status === 'Scan' && activeMatch.qr) {
          setQrString(activeMatch.qr);
        }
      }
    });
    ...
    socketRef.current.on('status_change', (data) => {
      ...
      setProfileAccounts(prev => prev.map(p => 
        p.id === data.instanceId 
          ? { ...p, status: data.status, qr: data.status === 'Connected' ? '' : p.qr } 
          : p
      ));
    });
  ...
  const handleImageAttach = (e) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      showNotification("Only JPG, PNG, GIF, or WEBP images are supported.", "warning");
      return;
    }
  ...
  const changeProfileTerminalContext = (instanceId) => {
    ...
    if (targetProfile && targetProfile.status === 'Scan' && targetProfile.qr) {
  ...
  const handleAiRecommendation = async () => {
    if (!aiContext.trim()) return showNotification("Please write what your business message is about first.", "warning");
    setAiLoading(true);
    try {
      const identityJwtToken = await auth.currentUser?.getIdToken();
  ...
  const executeMasterCampaignPipeline = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        showNotification(isScheduledCampaign ? "Campaign scheduled successfully!" : "Broadcast started successfully!", "success");
      } else {
        showNotification(data.error || "Failed to process transmission pipeline.", "error");
      }
    } catch (err) { showNotification("Failed to process transmission pipeline.", "error"); }
  };
        ...
        <div style={styles.authModalOverlay}>
          <form style={styles.authGlassForm} onSubmit={handleLogin}>
            <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
              ...
              <h2 style={{ fontSize: '22px', margin: 0, fontWeight: '600', color: '#e9edef', letterSpacing: '-0.5px' }}>Broadcast Enterprise</h2>
            </div>
          ...
          <div onClick={(e) => e.stopPropagation()} className="modal-animation" style={styles.modalContentGlassBox}>
            <div style={styles.modalHeaderGroup}>
              <div>
                ...
                <p style={styles.modalSubtitleText}>Scan this code from your mobile WhatsApp Linked Devices menu</p>
              </div>
            ...
            <div style={styles.modalFooterBadge}>
              ...
              <span style={{ fontSize: '12px', color: '#aebac1', fontWeight: '500' }}>Active Token Stream: {selectedInstanceId.toUpperCase()}</span>
            </div>
          </div>
        </div>
      )}
          ...
          <aside className="wa-sidebar-element" style={styles.waLeftSidebar}>
                ...
                >
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '5px', flex: 1, minWidth: 0, paddingRight: '40px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <h4 style={{ ...styles.waRowTitle, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '140px' }}>
                        {profile.name}
                      </h4>
                  ...
                  <div className="row-action-controls">
                    ...
                    <button 
                      ...
                      title="Terminate Node Session Link"
                    >
                      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div style={styles.sidebarQrFooterZone}>
              ...
              <div 
                ...
                title={qrString ? "Click to maximize QR pairing frame" : ""}
              >
                {qrString ? (
                  ...
                  <div style={{ color: connectionStatus.toLowerCase() === 'connected' ? '#00a884' : '#8696a0', fontSize: '12.5px', fontWeight: '500', textAlign: 'center', padding: '0 10px', lineHeight: '1.4' }}>
                    {connectionStatus.toLowerCase() === 'connected' ? '✅ Token Synchronized' : connectionStatus.toLowerCase() === 'initializing' ? '⏳ Preparing System Engine...' : 'Awaiting node synchronization...'}
                  </div>
                )}
              </div>
            </div>
          ...
          <main style={styles.waContentPanel}>
            <div style={styles.mainScrollableBody} className="custom-scrollbar">
              ...
              <section style={styles.studioCardSection}>
                ...
                <p style={styles.sectionSubtitleText}>Upload target contact list via Excel or CSV file.</p>
                <div style={styles.fileUploaderArea}>
                  ...
                  <div style={{ fontSize: '13px', color: parsedRows.length > 0 ? '#00a884' : '#8696a0', fontWeight: parsedRows.length > 0 ? '500' : '400' }}>
                    {uploadLoading ? "Reading dynamic columns..." : parsedRows.length > 0 ? `✅ Ready: ${parsedRows.length} contacts buffered` : "No active file source loaded"}
                  </div>
                </div>
              </section>
              ...
              {showAiConsole && (
                <section style={styles.aiAssistDockContainer}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 140px', gap: '10px' }}>
                    <input type="text" className="input-focus-effect" value={aiContext} onChange={(e) => setAiContext(e.target.value)} placeholder="Describe campaign copy requirements guidelines..." style={styles.waInputStyle} />
                  ...
                  <button onClick={handleAiRecommendation} disabled={aiLoading} className="interactive-btn" style={{ ...styles.aiActionButtonRun, marginTop: '10px' }}>
                    {aiLoading ? 'Synthesizing template params...' : 'Generate Parameters'}
                  </button>
                </section>
              )}
              ...
              <section style={styles.studioCardSection}>
                ...
                <p style={styles.sectionSubtitleText}>Inject structural target placeholder tags and load system attachment assets.</p>
                
                <div style={styles.tokenButtonRack}>
                  <button onClick={() => injectTagToken('Name')} className="interactive-btn" style={styles.tokenTagItem}>+ Name Tag</button>
                  <button onClick={() => injectTagToken('Company')} className="interactive-btn" style={styles.tokenTagItem}>+ Company Tag</button>
                  {parsedRows.length > 0 && Object.keys(parsedRows[0]).map(k => !['Phone', 'phone', 'Name', 'Company'].includes(k) && (
                    <button key={k} onClick={() => injectTagToken(k)} className="interactive-btn" style={styles.tokenTagItem}>+ {k}</button>
                  ))}
                </div>
              ...
              <section style={styles.schedulerWrapperBox}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <label htmlFor="broadcast-toggle" style={{ color: '#e9edef', fontSize: '13px', fontWeight: '500', cursor: 'pointer' }}>⏰ Defer bulk campaign execution timeframe (Schedule campaign execution)</label>
                </div>
                {isScheduledCampaign && (
                  <div style={{ marginTop: '12px', display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <span style={{ fontSize: '12.5px', color: '#8696a0' }}>Trigger Execution Time:</span>
              ...
              <button onClick={executeMasterCampaignPipeline} disabled={parsedRows.length === 0} className="interactive-btn" style={styles.waPipelineSubmitActionBtn(parsedRows.length > 0)}>
                {isScheduledCampaign ? 'Commit Operational Array To Queue Database' : 'Launch Mass Bulk Distribution'}
              </button>
            </div>
          </main>
        </div>
...
const styles = {
  ...
  authLogoIconWrapper: { display: 'inline-flex', backgroundColor: 'rgba(0, 168, 132, 0.12)', padding: '12px', borderRadius: '12px', marginBottom: '1rem', boxShadow: '0 8px 20px rgba(0,0,0,0.15)' },
  ...
  modalCloseXButton: { background: 'transparent', border: 'none', color: '#8696a0', fontSize: '16px', cursor: 'pointer', padding: '4px', outline: 'none', transition: 'color 0.15s' },
  ...
  waRowTitle: { margin: 0, fontSize: '14px', fontWeight: '500', color: '#e9edef', letterSpacing: '-0.2px' },
  ...
  sidebarQrFooterZone: { padding: '16px', backgroundColor: '#202c33', borderTop: '1px solid rgba(255,255,255,0.03)', flexShrink: 0, width: '100%', boxSizing: 'border-box' },
  ...
  qrCanvasWrapper: { backgroundColor: '#fff', padding: '6px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' },
  ...
  studioCardSection: { backgroundColor: '#111b21', borderRadius: '10px', padding: '20px', border: '1px solid #222e35', boxShadow: '0 4px 20px rgba(0,0,0,0.15)' },
  ...
  tokenButtonRack: { display: 'flex', gap: '8px', marginBottom: '12px', overflowX: 'auto', paddingBottom: '4px' },
  tokenTagItem: { backgroundColor: 'rgba(32, 44, 51, 0.6)', border: '1px solid rgba(255,255,255,0.06)', color: '#53bdeb', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: '500' },
  ...
  imageRemoveButton: { backgroundColor: 'transparent', border: 'none', color: '#ea0038', cursor: 'pointer', fontSize: '12.5px', marginLeft: 'auto', fontWeight: '500' },
