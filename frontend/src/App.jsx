          ...
          <main style={styles.waContentPanel}>
            <div style={styles.mainScrollableBody} className="custom-scrollbar">
              ...
              <section style={styles.studioCardSection}>
                ...
                {/* ── FIX: ACTUAL FILE DATA TABLE PREVIEW GRID ── */}
                {parsedRows && parsedRows.length > 0 && (
                  <div className="dataset-scroll-container custom-scrollbar" style={{ marginTop: '16px', borderRadius: '8px', border: '1px solid #222e35', backgroundColor: 'rgba(11, 20, 26, 0.4)', overflowX: 'auto', maxHeight: '240px' }}>
                    <table className="dataset-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textFillColor: '#d1d7db' }}>
                      <thead>
                        <tr style={{ backgroundColor: '#202c33', textAlign: 'left' }}>
                          {excelColumns.map((header) => (
                            <th key={header} style={{ padding: '10px 14px', color: '#8696a0', fontWeight: '500', borderBottom: '1px solid #222e35', whiteSpace: 'nowrap' }}>
                              {header}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {parsedRows.slice(0, 5).map((row, index) => (
                          <tr key={index} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
                            {excelColumns.map((header) => (
                              <td key={header} style={{ padding: '10px 14px', color: '#d1d7db', whiteSpace: 'nowrap' }}>
                    ...
                    {parsedRows.length > 5 && (
                      <div style={{ padding: '8px 14px', fontSize: '11px', color: '#8696a0', backgroundColor: '#111b21', textAlign: 'center', borderTop: '1px solid #222e35' }}>
                        Previewing top 5 rows out of {parsedRows.length} total rows cached.
                      </div>
                    )}
                  </div>
                )}
